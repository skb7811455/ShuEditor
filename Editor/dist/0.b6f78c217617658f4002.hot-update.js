...
function File(editor) {
    this.editor = editor;
    this.$elem = $("<div><img src='add.jpg'><input type='file'></img><button type='button'><i class='fa fa-file'></i></button><button><i class='fa fa-camera'></i></button></div>").addClass("s-e-file");
    this.type = 'click';
...
File.prototype = {
    ...
    readFile: function () {
        ...
        const file = this.$elem.ele.firstChild.firstChild.files[0];
    ...
    _bindEvent: function () {
        ...
        $elem.on(this.type, function (event) {
            if (event.target.type === "button") {
                if ($elem.ele.firstChild.firstChild.files[0]) {
                    that.readFile(that);
                }
